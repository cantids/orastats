...
class Main(OraStats):
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            'orastats', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser.add_argument('-g', '--group', default='all',
                            help="Check Database Group, all", required=False)
        parser.add_argument('-n', '--node', default='all',
                            help="Check Database Ip, all", required=False)
        parser.add_argument('-s', '--servicename',
                            help="Database Service Name", required=False)
        parser.add_argument('-t', '--type', default='sqlite3',
                            help="Database Type sqlnite,oracle,mysql", required=False)
        parser.add_argument('--savedb', action="store_true", dest="savedb",
                            default=False, help="Save Query Data to database")
        ...
        parser_a = subparsers.add_parser(
            'add', help='add Database Information', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser_a.add_argument('-ng', '--newgroup',
                              help="Database Group Name", required=True)
        parser_a.add_argument(
            '-ni', '--newip', help="Database Ip Address", required=True)
        parser_a.add_argument('-np', '--newport',
                              help="Database Port ", required=True)
        parser_a.add_argument('-ns', '--newservicename',
                              help="Database Service Name", required=True)
        parser_a.add_argument(
            '-nu', '--newusername', help="Database Username with sys views grant", required=True)
        parser_a.add_argument('-nP', '--newpasswd',
                              help="Database Username Password", required=True)

        parser_d = subparsers.add_parser(
            'del', help='del database Information', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser_d.add_argument('-ng', '--newgroup',
                              help="Database Group Name", required=True)
        parser_d.add_argument(
            '-ni', '--newip', help="Database Ip Address", required=True)
        parser_d.add_argument('-np', '--newport',
                              help="Database Port ", required=True)
        parser_d.add_argument('-ns', '--newservicename',
                              help="Database Service Name", required=True)
        parser_d.add_argument('-nu', '--newusername', default='',
                              help="Database Username with sys views grant", required=False)
        parser_d.add_argument('-nP', '--newpasswd', default='',
                              help="Database Username Password", required=False)

        parser_i = subparsers.add_parser(
            'init', help='init Database and create table ora_db_info', )
        parser_i.add_argument('-t', '--type', default='sqlnite',
                              help="Database Type sqlnite,oracle,mysql", required=False)

        parser_l = subparsers.add_parser(
            'list', help='list Database ', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser_l.add_argument('-g', '--group', default='all',
                              help="Database Group Name", required=False)
        parser_l.add_argument('-n', '--node', default='all',
                              help="Database Ip, all", required=False)
        parser_l.add_argument('-s', '--servicename',
                              help="Database Service Name", required=False)

        # for automatic add tablespace datafile
        parser_add = subparsers.add_parser(
            'addfile', help='automatic add tablespace datafile', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser_add.add_argument('-tw', '--tbswar',    default=80,
                                help="Tablespace Waring Pct ", required=False)
        parser_add.add_argument('-dn', '--dbfnum',    default=1,
                                help="add Datafile number", required=False)
        parser_add.add_argument(
            '-as', '--asmfsize',  default=61440,  help="asm free size(Mb)", required=False)
        parser_add.add_argument(
            '-ds', '--dbfsize',   default=20480,  help="Database Port ", required=False)
        parser_add.add_argument('-tn', '--tbsname',   default=None,
                                help="Database Tablespace Name ", required=False)
        parser_add.add_argument('-test', '--test',    action="store_true",
                                default=False, help="Database Tablespace Name ", required=False)

        parser_add = subparsers.add_parser(
            'savedbadd', help='save query data to database,only support mysql ', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser_add.add_argument('--sip',         default='127.0.0.1',
                                help="Mysql Database Machine IP", required=True)
        parser_add.add_argument('--sport',       default=3306,
                                help="Mysql Database Machine Port", required=True)
        parser_add.add_argument('--sdbname',     default='oramon',
                                help="Mysql Database Name", required=True)
        parser_add.add_argument(
            '--susername',   default='zabbix',         help="Database Port ", required=True)
        parser_add.add_argument('--spwd',        default='zabbix',
                                help="Database Tablespace Name ", required=True)
        parser_add.add_argument('--scharset',    default='UTF8',
                                help="Database Tablespace Name ", required=True)
    ...
    def db_add(self):
        ...
        params = (self.args.newgroup,
                  self.args.newip,
                  self.args.newport,
                  self.args.newservicename,
                  self.args.newusername,
                  self.args.newpasswd
                  )
        ...
        try:
            self.oracur.execute(sql, params)
    ...
    def db_del(self):
        sql = "delete from ora_db_info where DB_GROUP = '" + self.args.newgroup + "' and db_ipaddr = '" + self.args.newip + \
            "' and db_port = '" + self.args.newport + \
            "' and db_servicename='" + self.args.newservicename + "'"
        try:
    ...
    def db_list(self):
        if self.args.node != 'local':
            ...
            rows = [(self.args.username, self.args.passwd,
                     self.args.ipaddress, self.args.port, self.args.database)]
        return rows
        ...
        try:
            ...
            if self.args.stat == "add":
                ...
                for row in self.db_list():
                    print('Group : [\033[1;31;40m%-10s\033[0m] Database : [\033[1;31;40m%-15s\033[0m] Port : [\033[1;31;40m%-6s\033[0m] Service_Name : [\033[1;31;40m%-10s\033[0m]' % (
